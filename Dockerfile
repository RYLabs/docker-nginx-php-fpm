...
# starting from the official PHP/FPM image
...
# adding on nginx using commands from the official Dockerfile
...
# add nginx user to www-data group, so it can access the fpm socket
...
ADD .dockerdev/php-fpm/zzz-docker.conf /usr/local/etc/php-fpm.d/zzz-docker.conf
